...
import com.projects.realestate.model.*;
import com.projects.realestate.repository.*;
import com.projects.rebook.service.NewsItemService;

import com.projects.rebook.utils.DateTimeUtils;
import com.projects.rebook.constant.Constant;
...
//  @Async("asyncExecutor")
  public CommonResponse crawlerBatDongSan() throws IOException {
    ...
    try {
      Document doc = Jsoup.connect(Constant.FOR_SALE).get();
      ...
      for (Element item : elements) {
        ...
        Element productDetail = document.getElementById(Constant.NEW_DETAILS_TAG);

        if (productDetail.getElementsByClass(Constant.LOCATION_PROP) != null) {
          String khuvuc = productDetail.getElementsByClass(Constant.LOCATION_PROP).select("a").text();
          newsItem.setCity(khuvuc);
        }

        String price = productDetail.getElementsByClass(Constant.PRICE_PROP).first().select("strong").text();
        String area = productDetail.getElementsByClass(Constant.PRICE_PROP).get(1).select("strong").text();
        newsItem.setPrice(price);
        newsItem.setArea(area);

        String desc = productDetail.getElementsByClass(Constant.DESCRIPTION_PROP).text()
            .replaceAll("<br>", "\t");
        newsItem.setDescription(desc);

        if (productDetail.getElementById(Constant.ROOM_NUMBER) != null) {
          String room = productDetail.getElementById(Constant.ROOM_NUMBER).getElementsByClass("right")
              .text();
          newsItem.setRoom_number(room);
        }

        if (productDetail.getElementById(Constant.TOILET_NUMBER) != null) {
          String toilet = productDetail.getElementById(Constant.TOILET_NUMBER).getElementsByClass("right")
              .text();
          newsItem.setToilet_number(toilet);
        }

        if (productDetail.getElementById(Constant.DIRECTOFHOUSE) != null) {
          String huongNha = productDetail.getElementById(Constant.DIRECTOFHOUSE).getElementsByClass("right")
              .text();
          newsItem.setDirect_of_house(huongNha);
        }

        if (productDetail.getElementById(Constant.BALCONY) != null) {
          String balcony = productDetail.getElementById(Constant.BALCONY).getElementsByClass("right").text();
          newsItem.setBalcony(balcony);
        }

        if (productDetail.getElementById(Constant.FLOOR_NUMBER) != null) {
          String floorNumber = productDetail.getElementById(Constant.FLOOR_NUMBER)
              .getElementsByClass("right").text();
          newsItem.setFloor_number(floorNumber);
        }

        if (productDetail.getElementById(Constant.WARDIN) != null) {
          String wardin = productDetail.getElementById(Constant.WARDIN).getElementsByClass("right").text();
          newsItem.setWardin(wardin);
        }

        if (productDetail.getElementById(Constant.FRONT_END) != null) {
          String frontEnd = productDetail.getElementById(Constant.FRONT_END).getElementsByClass("right")
              .text();
          newsItem.setFrontEnd(frontEnd);
        }

        if (productDetail.getElementById(Constant.INTERIOR) != null) {
          String interior = productDetail.getElementById(Constant.INTERIOR).getElementsByClass("right")
              .text();
        ...
        if(productDetail.getElementById(Constant.OWNER_NAME) != null) {
          String owner = productDetail.getElementById(Constant.OWNER_NAME).getElementsByClass("right").text();
          contactOwner.setContactName(owner);
        }
        if (productDetail.getElementById(Constant.OWNER_PHONE) != null) {
          String phone = productDetail.getElementById(Constant.OWNER_PHONE).getElementsByClass("right").text();
          contactOwner.setPhoneNumber(phone);
        }
        if (productDetail.getElementById(Constant.OWNER_EMAIL) != null) {
          String email = productDetail.getElementById(Constant.OWNER_EMAIL).getElementsByClass("right")
              .select("a").text();
          contactOwner.setEmail(email);
        }
        if (productDetail.getElementById(Constant.OWNER_ADDRESS) != null) {
          String address = productDetail.getElementById(Constant.OWNER_ADDRESS).getElementsByClass("right")
              .text();
        ...
        Element project = productDetail.getElementById(Constant.PROJECT);
        if (project != null) {
          ...
          if (project.getElementById(Constant.PROJECT_OWNER) != null) {
            String projectOwner = project.getElementById(Constant.PROJECT_OWNER).getElementsByClass("right")
                .text();
            propertyProject.setProjectOwner(projectOwner);
          }
          if (project.getElementById(Constant.PROJECT_SIZE) != null) {
            String projectSize = project.getElementById(Constant.PROJECT_SIZE).getElementsByClass("right")
                .text();
...
//  @Async("asyncExecutor")
  public CommonResponse crawlerDiaOcOnline() throws IOException {
    ...
    try {
      Document doc = Jsoup.connect(Constant.DIAOCONLINE_DUAN_QUYHOACH).get();
